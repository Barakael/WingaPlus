...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose, activeTab, onTabChange }) => {
    ...
    switch (user?.role) {
      ...
      case 'shop_owner':
        return [
          ...
          { id: 'categories', label: 'Categories', icon: QrCode },
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`
        w-64 bg-white dark:bg-gray-800 shadow-xl transform transition-transform duration-300 ease-in-out lg:translate-x-0 flex flex-col h-full
      ...
      `}>
        ...
        <nav className="mt-8 px-4 overflow-y-auto flex-1 pb-4">
