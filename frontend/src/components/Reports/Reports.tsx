...
const Reports: React.FC = () => {
  ...
  const downloadWarrantyReport = () => {
    const warrantyData = sales.filter(sale => sale.warranty_months > 0).map(sale => {
      const product = products.find(p => p.id === sale.product_id);
      return {
        'Sale ID': sale.id,
        'Product': product?.name || 'Unknown',
        ...
        'Warranty Months': sale.warranty_months,
        'Sale Date': sale.sale_date,
        'Expiry Date': new Date(new Date(sale.sale_date).getTime() + (sale.warranty_months * 30 * 24 * 60 * 60 * 1000)).toISOString().split('T')[0]
      };
    });
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <button
            ...
            className="p-4 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-lg hover:from-green-600 hover:to-green-700 transition-all duration-200 flex items-center justify-center"
          >
            <Download className="h-5 w-5 mr-2" />
            Inventory Report
          </button>
